        ...
        if (ban.getBoolean(player.getName() + ".ban")){
            if (!ban.getBoolean(player.getName() + ".perm")){
                ...
                if (currentTime > ban.getInt(player.getName() + ".timeEnd")){
                    ...
                    saveBan();
                    e.disallow(PlayerLoginEvent.Result.KICK_BANNED, "You have been temporarily banned from the server! "  + ChatColor.AQUA + "Duration: " + ChatColor.DARK_AQUA + durationMsg + ChatColor.WHITE + " Reason: "  + ChatColor.DARK_RED + ban.getString(player.getName() + ".reason"));
                }
            } else {
                e.disallow(PlayerLoginEvent.Result.KICK_BANNED, "You have been banned from the server! Reason: "  + ChatColor.DARK_RED + ban.getString(player.getName() + ".reason"));
            }
        }
    }
